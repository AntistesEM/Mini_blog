  ...
  return (
    <div className="card mb-3">
      <div className="card-body">
        ...
        <p className="card-text">{post.text.slice(0, 100)}{post.text.length > 100 ? '...' : ''}</p>
        <Link to={`/posts/${post.id}`} className="btn btn-link">Читать</Link>
        <Button variant="info" onClick={() => onEdit(post)}>Редактировать пост</Button>
        <Button
          variant="danger"
